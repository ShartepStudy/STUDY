...
DWORD WINAPI Decoding_Thread(LPVOID lp)
{
	...
	if(dwAnswer == WAIT_OBJECT_0)
	{
		...
		if(!in)
		{
			MessageBox(ptr->hDialog, TEXT("Ошибка открытия файла!"), TEXT("decoding"), MB_OK | MB_ICONINFORMATION);
			return 1;
		}
		...
		MessageBox(ptr->hDialog, 
				TEXT("Чтение данных из файла coding.txt завершено!"), TEXT("Семафор"), MB_OK | MB_ICONINFORMATION);
	}
	return 0;
}
